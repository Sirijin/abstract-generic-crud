...
public class ProjectService extends AbstractService<Project, ProjectRepository, ProjectMapper> {
    ...
    public ProjectDto addPersonsToProject(Long projectId, Set<Long> personIds) {
        Project project = super.repository.findById(projectId).orElseThrow(() ->
                new SampleException(String.format(ErrorType.ENTITY_NOT_FOUND.getDescription(), projectId))
        );
        ...
        if (persons.size() != personIds.size()) {
            throw new SampleException("One or more person entities do not exist");
        }
